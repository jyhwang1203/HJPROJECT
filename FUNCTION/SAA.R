...
MVO_VOLTARGET  <- function(mu,cov,trate,n,wl,vol,CT){
  ...
  names(result$par) <-c("MSKRT","KRBONDH","WORLDT","USBOND","WREPRA","WRINFRA")
...
mu24
VOL
MVO_VOLTARGET(mu24,cov24,trate= 0.0523 ,n=6,wl=0.04,0.12,CT=0.3)%>%as.matrix()
MVO_VOLTARGET(mu24,cov24,trate= 0.0480 ,n=6,wl=0.04,0.0953 ,CT=0.5 )%>%as.matrix()
MVO_VOLTARGET(mu24,cov24,trate= 0.0428 ,n=6,wl=0.04,0.0749 ,CT=0.7 )%>%as.matrix()
weiv1<- MVO_VOLTARGET(mu24,cov24,trate= 0.0523 ,n=6,wl=0.04,0.12,CT=0.3)%>%as.matrix()
weiv2<- MVO_VOLTARGET(mu24,cov24,trate= 0.0480 ,n=6,wl=0.04,0.0953 ,CT=0.5 )%>%as.matrix()
weiv3<- MVO_VOLTARGET(mu24,cov24,trate= 0.0428 ,n=6,wl=0.04,0.0749 ,CT=0.7 )%>%as.matrix()
