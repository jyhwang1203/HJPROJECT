...
RAWDATA%>%filter(variable=="SP500"|variable=="KOSPI"|variable=="USGOVT"|variable=="KRBOND")%>%filter(STD_DT>"2000-01-01")%>%dcast(STD_DT~variable)%>% trans_rt("year")%>%dt_trans%>%graphbar2("WEEK")
...
ui <-  navbarPage("Dashboard", theme = shinytheme("flatly"),
       tabPanel("기준금리",
        ...
        dashboardSidebar(dateRangeInput('range',label = '',start = as.Date('1970-01-01') , end = as.Date('2023-12-31')),
                         checkboxGroupInput("labor", "Variables to show:",c("미국실업률"="USLABOR","임금인상(YoY)"="AHEYOY","임금인상(MoM)"="AHEMOM","미국기준금리리"="USBR","장단기스프레드"="SP"),selected="USLABOR"),
                         checkboxGroupInput("NF", "Variables to show:",c("비농업고용지수"="NFPTCH","ADP고용"="ADPNFP"),selected="NFTCH")),
        dashboardBody(fluidRow(box(plotOutput("lab"), width =12, solidHeader = TRUE)),
                      fluidRow(box(plotOutput("NF"), width =12, solidHeader = TRUE))
                      ))))

server <- function(input, output){
   output$lab   <- renderPlot({
   RAWDATA%>%filter(variable=="US10Y"|variable=="US2Y"|variable==input$labor)%>%filter(STD_DT>input$range[1]&STD_DT<input$range[2])%>%dcast(STD_DT~variable)%>%mutate(SP=US10Y-US2Y)%>%mutate(Base=min(USLABOR))%>%
          dplyr::select(-c(US10Y,US2Y))%>%exname%>%cplot+theme(legend.text = element_text(size=15))
   })
