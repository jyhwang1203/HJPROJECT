...
CANARY <- RAWDATA%>%select(STD_DT,USBOND,SP500,DM,EM)%>%fillf%>%na.omit%>%trans_rt("month")%>%dt_trans()
DATE <- CANARY
"2001-05-30"
"2012-06-30"
...
#POFF <-SMA %>%select(STD_DT,QQQ,VWO,VEA,AGG)
...
UNIV_FINAL2<- UNIV%>%left_join(RES3,by=c("STD_DT_L"="STD_DT"))%>%left_join(RES6,by=c("STD_DT_L"="STD_DT"))

INDEXDEF<- UNIV_FINAL%>%filter(state==0)%>%select(STD_DT)%>%left_join(INDEXDEF,by=c("STD_DT"="STD_DT"))
INDEXOFF<- UNIV_FINAL%>%filter(state==1)%>%select(STD_DT)%>%left_join(INDEXOFF,by=c("STD_DT"="STD_DT"))
INDEX <- rbind(INDEXDEF,INDEXOFF)%>%arrange(STD_DT)%>%left_join(TEMP%>%melt(id.vars="STD_DT"),by=c("STD_DT","INDEX"="variable"))%>%left_join(UNIV_FINAL%>%select(STD_DT,state),by="STD_DT")
...
BAA_RT2<- data.frame(rbind(UNIV_FINAL2%>%filter(state==0)%>%select(STD_DT_L,DEF)%>%dplyr::rename(RT="DEF"),
                          UNIV_FINAL2%>%filter(state==1)%>%select(STD_DT_L,OFF)%>%dplyr::rename(RT="OFF")))%>%arrange(STD_DT_L)%>%na.omit



RT_BAA<- cuml(retm%>%select(STD_DT,BM) %>% left_join(BAA_RT,by=c("STD_DT"="STD_DT_L"))%>% left_join(BAA_RT2,by=c("STD_DT"="STD_DT_L"))%>%na.omit)
RT_BAA2<- cuml(retm%>%select(STD_DT,BM) %>% left_join(BAA_RT2,by=c("STD_DT"="STD_DT_L"))%>%na.omit)
RET_BAA <- retm%>%select(STD_DT,BM) %>% left_join(BAA_RT,by=c("STD_DT"="STD_DT_L"))%>% left_join(BAA_RT2,by=c("STD_DT"="STD_DT_L"))

cuml(retm%>%select(STD_DT,BM) %>% left_join(BAA_RT,by=c("STD_DT"="STD_DT_L"))%>% left_join(BAA_RT2,by=c("STD_DT"="STD_DT_L"))%>%na.omit) %>% melt(id.vars="STD_DT")%>%ggplot(aes(STD_DT, value, col = variable)) +             
  geom_line(size=1)+ 
  ggtitle("누적수익률") +
  theme(legend.text = element_text(size=15))

PA_BAA<- RET_BAA %>%na.omit %>%PA   
...
MYFUN <- function(data){return((prod(1+data)-1)%>%round(4))}
RET_BAA%>%mutate(YEAR=substr(STD_DT,1,4))%>%aggregate(cbind(BM,RT)~YEAR,.,FUN=MYFUN)
