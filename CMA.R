...
pkg <-c("readxl","data.table","dplyr","zoo","ggplot2","PerformanceAnalytics","writexl","plyr","quantmod",
        "flexdashboard","gridExtra","tidyverse","knitr","lubridate","tseries","nloptr","riskPortfolios","ggcorrp Flot","mvtnorm",'xlsx',"cvar")
ipak(pkg)
...
##########################################################################################################################################################
      ...
      STDDT-years(5)
      COVLONG  <- retm%>%filter(STD_DT>(STDDT-years(5))) %>%select(WORLD,MSKR,WRBOND,KRBOND,AL,GSCI)%>%cov    
