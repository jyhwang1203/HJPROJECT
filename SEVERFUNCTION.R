server <- function(input, output){
  ...
  output$Plot2 <- renderPlot({
    ggplot(retsy, mapping = aes_string(x = retsy$STD_DT,
                                       y = input$variableh)) +
  ...
  output$vixtable <- renderDT({
    ...
    # vixtable<-  macro %>% select(STD_DT,VIX)%>%
    #   right_join(HG%>%select(STD_DT,HMACRO,WORLD)%>%trans_rt("quater")%>%dt_trans,by="STD_DT")%>%na.omit%>%
    #   mutate(percent_rank =ntile(VIX,4))%>%
    #   group_by(percent_rank)%>%dplyr::summarise(HMACRO=mean(HMACRO),WORLD=mean(WORLD),VIX=mean(VIX)) %>%round(3)
